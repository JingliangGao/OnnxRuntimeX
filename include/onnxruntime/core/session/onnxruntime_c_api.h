...
#ifdef USE_PHYNPU
  /** \brief Used for custom operators, set the output ptr of a kernel
   * \param[in] index See KernelContext_SetPhyOutput for boundaries check.
   * \param[in] ort_value a ptr to OrtValue to output otherwise set to nullptr
   * \see ::OrtCustomOp
   */
  ORT_API2_STATUS(KernelContext_SetPhyOutput, _In_ const OrtKernelContext* context, _In_ size_t index, 
                  _In_ const OrtValue* ort_value, _Out_ int *out_arg_index);

  ORT_API2_STATUS(KernelContext_SetPhyInput, _In_ const OrtKernelContext* context, _In_ size_t index, 
                  _In_ const OrtValue* ort_value, _Out_ int *in_arg_index);
#endif
