...
#ifdef USE_PHYNPU
inline size_t KernelContext::SetPhyOutput(size_t index, const OrtValue* ort_value, int *out_arg_index) const {
  Ort::ThrowOnError(GetApi().KernelContext_SetPhyOutput(ctx_, index, ort_value, out_arg_index));
  return 0;
}

inline size_t KernelContext::SetPhyInput(size_t index, const OrtValue* ort_value, int *in_arg_index) const {
  Ort::ThrowOnError(GetApi().KernelContext_SetPhyInput(ctx_, index, ort_value, in_arg_index));
  return 0;
}
#endif
