...
#ifdef USE_PHYNPU
ORT_API_STATUS_IMPL(KernelContext_SetPhyOutput, _In_ const OrtKernelContext* context, _In_ size_t index, _In_ const OrtValue* ort_value, _Out_ int *out_arg_index);
ORT_API_STATUS_IMPL(KernelContext_SetPhyInput, _In_ const OrtKernelContext* context, _In_ size_t index, _In_ const OrtValue* ort_value, _Out_ int *in_arg_index);
#endif
