...
#ifdef USE_PHYNPU
    {
      auto source_unspoort_node_file = GetUnspportOpsFile();
      if(!source_unspoort_node_file.empty() || source_unspoort_node_file != "") {
        LOGS(*session_logger_, INFO) << "GetUnspportOpsFile:"<<source_unspoort_node_file;
        std::remove(PHYNPU_UNSPPORTED_NODES_FILE.c_str());  
        std::ifstream sourceFile(source_unspoort_node_file);
        std::ofstream destinationFile(PHYNPU_UNSPPORTED_NODES_FILE);
        std::string line;
        ...
        LOGS(*session_logger_, INFO) << "Not GetUnspportOps File Path.";
      }
    }
#endif
