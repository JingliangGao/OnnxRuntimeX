...
TensorShape broadcast_shape(const TensorShape& shape1, const TensorShape& shape2) {
  ...
  for (size_t i = 0; i < max_dims; ++i) {
    ...
    if (dim1 != dim2 && dim1 != 1 && dim2 != 1) {
      ...
      auto print_dims = [&](const TensorShape& s){
        const auto& dims = s.GetDims();
        oss << "[";
      ...
      oss << "Shapes are not compatible for broadcasting at axis index (from left) = " << i
          << ", expanded dims=" << dim1 << ", dim2=" << dim2
          << ",shape1="; print_dims(shape1);
      oss << ",shape2="; print_dims(shape2);
  ...
  class CLASS_NAME : public OpenCLKernel {                                                                              \
   public:                                                                                                              \
    ...
    Status Compute(OpKernelContext* context) const override {                                                           \
      ...
      int64_t constant_shape[MAX_DIMS] = {};                                                                            \
      auto C_Shape = exec_->GetScratchBufferTmp(Shape_size);                                                            \
      const int64_t* a_Shape = (a->Shape().GetDims().empty()) ? constant_shape : a->Shape().GetDims().data();           \
      const int64_t* b_Shape = (b->Shape().GetDims().empty()) ? constant_shape : b->Shape().GetDims().data();           \
      const int64_t* c_Shape = (c->Shape().GetDims().empty()) ? constant_shape : c->Shape().GetDims().data();           \
      ...
      ORT_RETURN_IF_ERROR(                                                                                              \
          KernelLauncher{GetKernel(#CLASS_NAME)}                                                                        \
              ...
              .SetArg<cl_int>((a->Shape().GetDims().empty()) ? 1 : a->Shape().NumDimensions())                \
              .SetArg<cl_int>((b->Shape().GetDims().empty()) ? 1 : b->Shape().NumDimensions())                \
              .SetArg<cl_int>((c->Shape().GetDims().empty()) ? 1 : c->Shape().NumDimensions())                \
