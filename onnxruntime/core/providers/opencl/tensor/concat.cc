...
Status Concat::Compute(OpKernelContext* ctx) const {
  ...
  for (size_t i = 0; i < p.inputs.size(); ++i) {
    err = clEnqueueWriteBuffer(exec_->GetCommandQueue(), In_shape, CL_TRUE, offset,
                         input_shape_size, p.inputs[i].tensor->Shape().GetDims().data(), 0, nullptr, nullptr);
    if(err != CL_SUCCESS){
      return Status(common::ONNXRUNTIME, common::FAIL, "Concat compute failed : clEnqueueWriteBuffer failed");
    }
