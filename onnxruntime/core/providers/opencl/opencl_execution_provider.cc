...
cl_mem OpenCLExecutionProvider::GetScratchBufferTmp(size_t nbytes) const {
  ...
  if (nbytes == 0) {
      LOGS_DEFAULT(ERROR) << "[CL] GetScratchBufferTmp: nbytes == 0";
      return nullptr;
  }
  cl_int err = CL_SUCCESS;
  cl_mem ptr = clCreateBuffer(this->GetOpenCLContext(), CL_MEM_READ_WRITE, nbytes, NULL, &err);
  if(err != CL_SUCCESS){
      LOGS_DEFAULT(ERROR) << "[CL] clCreateBuffer failed: err=" << err;
      return nullptr;
  }
...
void OpenCLExecutionProvider::ReadFromCLBuffer(cl_mem buffer,void* host_ptr,size_t nbytes){
      cl_int err;
    if (!buffer) {
        LOGS_DEFAULT(ERROR) << "[CL] ReadFromCLBuffer: buffer is null";
        return;
    }
    if (!host_ptr) {
        LOGS_DEFAULT(ERROR) << "[CL] ReadFromCLBuffer: host_ptr is null";
        return;
    }
    if (nbytes == 0) {
        LOGS_DEFAULT(ERROR) << "[CL] ReadFromCLBuffer: nbytes == 0, nothing to read";
        return;
    }
    err = clEnqueueReadBuffer(this->GetCommandQueue(), buffer, CL_TRUE, 0, nbytes, host_ptr, 0, nullptr, nullptr);
    if (err != CL_SUCCESS) {
        LOGS_DEFAULT(ERROR) << "[CL] clEnqueueReadBuffer failed: err=" << err;
        return;
    }
    err = clFinish(this->GetCommandQueue());
    if (err != CL_SUCCESS) {
        LOGS_DEFAULT(ERROR) << "[CL] clFinish failed: err=" << err;
        return;
    }
}
...
void OpenCLExecutionProvider::WriteToCLBuffer(cl_mem buffer, const void* host_ptr, size_t nbytes){
    cl_int err;
    if (!buffer) {
        LOGS_DEFAULT(ERROR) << "[CL] WriteToCLBuffer: buffer is null";
        return;
    }
    if (!host_ptr) {
        LOGS_DEFAULT(ERROR) << "[CL] WriteToCLBuffer: host_ptr is null";
        return;
    }
    if (nbytes == 0) {
        LOGS_DEFAULT(ERROR) << "[CL] WriteToCLBuffer: nbytes == 0, nothing to write";
        return;
    }
    err = clEnqueueWriteBuffer(this->GetCommandQueue(), buffer, CL_TRUE, 0, nbytes, host_ptr, 0, nullptr, nullptr);
    if (err != CL_SUCCESS) {
        LOGS_DEFAULT(ERROR) << "[CL] clEnqueueWriteBuffer failed: err=" << err;
        return;
    }
